...
esp_err_t _http_event_handler(esp_http_client_event_t *evt)
{
    ...
    case HTTP_EVENT_ON_DATA:
        ...
        if (handle->response_buffer.buffer == NULL) {
            handle->response_buffer.buffer = malloc(evt->data_len);
            ...
            handle->response_buffer.buffer = realloc(handle->response_buffer.buffer, new_len);
 ...
 */
static err_t do_https_request(const esp_dns_handle_t handle, const char *name, ip_addr_t *addr, int addrtype)
{
    uint8_t buffer_qry[BUFFER_SIZE];    // TBD: Make it a request specific stack buffer.
    ...
    int port = handle->config.protocol_config.doh_config.port ?
               handle->config.protocol_config.doh_config.port : DEFAULT_DOH_PORT;
    ...
    size_t query_size = create_dns_query(buffer_qry, sizeof(buffer_qry), name, addrtype, &handle->response_buffer.dns_response.id);
    if (query_size == -1) {
...
int init_doh_dns(esp_dns_handle_t handle)
{
    ...
    // Currently a stub implementation as ESP32 requires ESP-HTTP-Client setup
    //set_error(handle, DNS_ERR_NOT_IMPLEMENTED, "DNS over HTTPS not implemented yet");
...
err_t dns_resolve_doh(const esp_dns_handle_t handle, const char *name, ip_addr_t *addr, u8_t rrtype)
{
    ...
    return do_https_request(handle, name, addr, rrtype);
}
