...
err_t dns_resolve_dot(const esp_dns_handle_t handle, const char *name, ip_addr_t *addr, u8_t rrtype)
{
    ...
    timeout_ms = handle->config.protocol_config.dot_config.timeout_ms ? : DEFAULT_DOT_TIMEOUT_MS;
    dot_port = handle->config.protocol_config.dot_config.port ? : DEFAULT_DOT_PORT;
    ...
    len = esp_transport_write(transport,
                              dot_buffer,
                              query_size + 2,
                              handle->config.protocol_config.dot_config.timeout_ms);
    if (len < 0) {
