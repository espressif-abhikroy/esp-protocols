...
esp_dns_handle_t esp_dns_init(const esp_dns_config_t *config)
{
    ...
    default:
        set_error(handle, DNS_ERR_INVALID_PARAM, "Unsupported DNS protocol: %d", config->protocol);
        ret = -1;
        break;
    }

    if (ret != 0) {
        ESP_LOGE(TAG, "Failed to initialize DNS protocol %d: %s",
                 config->protocol, handle->error_message);
 ...
 */
int dns_get_last_error(void)
{
    if (g_dns_handle == NULL) {
        return DNS_ERR_INIT_FAILED;
    }

    return g_dns_handle->last_error;
}

/**
 * Get the last error message from the DNS module.
 *
 * @param buffer Buffer to store the error message
 * @param size Size of the buffer
 * @return 0 on success, non-zero on failure
 */
int dns_get_error_message(char *buffer, size_t size)
{
    if (g_dns_handle == NULL || buffer == NULL || size == 0) {
        return -1;
    }

    if (xSemaphoreTake(g_dns_handle->lock, portMAX_DELAY) != pdTRUE) {
        return -1;
    }

    snprintf(buffer, size, "%s", g_dns_handle->error_message);

    xSemaphoreGive(g_dns_handle->lock);
    return 0;
}
...
void set_error(esp_dns_handle_t handle, int error_code, const char *format, ...)
{
    ...
    ESP_LOGE(TAG, "DNS error %d: %s", error_code, handle->error_message);
}
...
{
    if (g_dns_handle == NULL) {
        ESP_LOGW(TAG, "ESP_DNS module not initialized, resolving through native DNS");
        *err = ERR_OK;
        return 0;
    }
